...
class Gen(nn.Module):
    def __init__(self, args, device):
        ...
        self.model = nn.Sequential(
            *block(self.input_dim + self.class_num, 256, normalize=False),
            *block(256, 512),
            *block(512, 1024),
            *block(1024, 2048),
            *block(2048, 4096),
            *block(4096,1024),
            *block(1024, self.output_dim),  # Final layer without activation
            # nn.Tanh()  # Optional: use an activation depending on your data
        )
